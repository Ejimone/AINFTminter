...
async function connectWallet() {
    if (typeof window.ethereum === 'undefined') {
        showNotification('error', 'MetaMask Not Found', 'Please install MetaMask extension to continue');
        return;
    }

    try {
        showNotification('info', 'Connecting...', 'Please approve the connection request in MetaMask');
        ...
        showNotification('success', 'Wallet Connected', `Connected to ${userAddress.slice(0, 6)}...${userAddress.slice(-4)}`);
        ...
        window.ethereum.on('accountsChanged', (accounts) => {
            if (accounts.length === 0) {
                ...
                document.getElementById('walletAddress').textContent =
                    userAddress.slice(0, 6) + '...' + userAddress.slice(-4);
                showNotification('info', 'Account Changed', 'Wallet account has been changed');
            }
        });
        ...
        showNotification('error', 'Connection Failed', error.message || 'Failed to connect to MetaMask');
    }
}
...
async function generateOnly() {
    ...
    try {
        ...
        setTimeout(() => {
            hideLoading();
            showNotification('success', 'NFT Generated', 'Your NFT has been generated successfully!');
            
            showResult('success', 'NFT Generated Successfully', `
                <div class="success-alert">
                    <strong>Success!</strong> Your NFT has been generated and uploaded to IPFS.
                </div>
                
                <div class="nft-preview">
                    <img src="${formatIpfsUrl(data.image_ipfs_uri)}" alt="${name}" class="nft-image" />
                </div>
            `);
...
document.getElementById('singleNftForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        setTimeout(() => {
            hideLoading();
            showNotification('success', 'NFT Minted', 'Your NFT has been successfully minted!');
...
document.getElementById('batchForm').addEventListener('submit', async (e) => {
    ...
    if (prompts.length === 0) {
        showNotification('error', 'Validation Error', 'Please enter at least one prompt');
        return;
    }
    
    if (prompts.length > 10) {
        showNotification('error', 'Too Many Prompts', `Maximum 10 prompts allowed. You entered ${prompts.length}`);
        return;
    }
    ...
    try {
        ...
        setTimeout(() => {
            ...
            showNotification('success', 'Batch Complete', `${successCount} images generated successfully`);
