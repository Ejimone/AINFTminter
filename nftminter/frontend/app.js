...
async function generateNFT(prompt, name, description, shouldMint = true) {
  ...
  try {
    ...
    showResult(
      "error",
      shouldMint ? "❌ Minting Failed" : "❌ Generation Failed",
      `
            <div class="error-alert">
                <strong>Error:</strong> ${error.message}
            </div>
            <p class="mt-2">
                ...
                <ul style="margin-top: 0.5rem; margin-left: 1.5rem; color: var(--text-secondary);">
                    ...
                    ${
                      shouldMint
                        ? "<li>MetaMask is connected and on Sepolia network</li>"
                        : ""
                    }
                </ul>
            </p>
...
function showNFTResult(data, wasMinted) {
  const explorerUrl = data.transaction_hash
    ? `https://sepolia.etherscan.io/tx/${data.transaction_hash}`
    : null;

  const title = wasMinted
    ...
    `
        ...
        <div class="nft-preview">
            <div class="nft-image-container">
                <img src="${formatIpfsUrl(data.image_ipfs_uri)}" alt="${
      data.name
    }" class="nft-image" />
                <div class="nft-overlay">
                    <button class="btn-view-fullsize" onclick="viewFullSize('${formatIpfsUrl(
                      data.image_ipfs_uri
                    )}', '${data.name}')">
                        🔍 View Full Size
                    </button>
        ...
        <div class="nft-metadata">
            ...
            <div class="metadata-item">
                ...
                <div class="metadata-value">
                    <a href="${formatIpfsUrl(
                      data.image_ipfs_uri
                    )}" target="_blank" class="metadata-link">
                        View on IPFS →
                    </a>
                </div>
            </div>
        ...
        <div class="nft-actions">
            <button class="btn-secondary" onclick="shareNFT('${
              data.name
            }', '${formatIpfsUrl(data.image_ipfs_uri)}', '${explorerUrl}')">
                📤 Share NFT
            </button>
            <button class="btn-primary" onclick="viewOnOpenSea('${
              data.contract_address
            }', '${data.token_id}')">
                🌊 View on OpenSea
            </button>
...
async function mintExistingNFT(metadataUri, name) {
  ...
  const shouldMint = confirm(
    `Do you want to mint "${name}" as an NFT on the blockchain?\n\nThis will require a small gas fee.`
  );
  if (!shouldMint) return;

  showLoading("Minting NFT", "Minting your NFT on the blockchain...", [
    "Minting on blockchain...",
  ]);
